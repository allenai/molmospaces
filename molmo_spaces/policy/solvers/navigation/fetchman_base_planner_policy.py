...
"""Port of g1_molmo's navigation policy: one live loop recomputing a
[vx, vy, yaw_rate] command from the current pose every step, emitted under
"base_velocity" (AStarPlannerPolicy instead pre-bakes rotate-then-drive
waypoints). Grid A* with wall-clearance cost, line-of-sight simplification and
the waypoint-following law are g1_molmo's; goal selection uses this codebase's
NavGoalSampler and AStarPlanner.map.
"""
