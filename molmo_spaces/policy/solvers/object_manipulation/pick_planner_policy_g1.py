...
# The rollout trace ("[G1_MOLMO_TRACE] ...", "[G1 walk] ...") that the gold
# parity gate compares byte for byte. Silent unless `enable_g1_trace()` is
# called: the comparison scripts and the interactive shell do, datagen does not.
G1_TRACE_LOGGER = "molmo_spaces.g1_trace"
trace_log = logging.getLogger(G1_TRACE_LOGGER)
trace_log.propagate = False
trace_log.setLevel(logging.WARNING)


def enable_g1_trace(stream=sys.stdout) -> None:
    """Print the G1 rollout trace as bare lines (the bytes gold's own print()
    statements produce) to `stream`."""
...
def prune_waypoints(waypoints, start_xy, reach=WAYPOINT_PRUNE_RADIUS):
    """Drop waypoints within `reach` of the start or of the previous kept one
    (the final waypoint always stays). The A* start snaps to the nearest free
    cell, often a few cm behind a robot standing in an inflation band, and
    turning in place to face it costs tens of cm of WBC drift.
    """
    if not waypoints:
        return waypoints
    goal = waypoints[-1]
    kept = []
    last = np.asarray(start_xy, dtype=np.float64)
    for wp in waypoints[:-1]:
        wp = np.asarray(wp, dtype=np.float64)
        if np.linalg.norm(wp - last) < reach:
            continue
        # Also skip a waypoint the goal itself already covers.
        if np.linalg.norm(wp - goal) < reach:
            continue
        kept.append(wp)
        last = wp
    kept.append(np.asarray(goal, dtype=np.float64))
    return kept


# A single-waypoint walk that starts within this distance of its goal is a
# short correction hop, not a walk: drive it holonomically (see holonomic_cmd).
HOLONOMIC_MAX_START_DIST = 1.0
HOLONOMIC_MAX_SPEED = 0.25
# Below ~0.05 the WBC's stand/walk switch never starts walking, and a hop that
# starts inside the brake zone would otherwise be commanded at millimetres per
# second. Overshoot at 0.1 m/s from the 0.09m stop radius is a few cm.
HOLONOMIC_MIN_SPEED = 0.10
HOLONOMIC_MAX_TURN = 0.3
# The hop is only taken when the goal lies within this half-angle of the
# robot's heading. Two mostly-sideways hops (base-frame vy ~1.6x vx) made the
# WBC unstable enough that MuJoCo reset the simulation -- the robot reappeared
# at the world origin -- while forward hops were fine; the reference law never
# commands more lateral than tan(FACE_WP_TOL)~0.25x forward either.
HOLONOMIC_MAX_BEARING = 0.35  # rad, ~20deg


def goal_in_forward_cone(xy, yaw, wp, half_angle=HOLONOMIC_MAX_BEARING):
    delta = np.asarray(wp, dtype=np.float64) - np.asarray(xy, dtype=np.float64)
    bearing = (np.arctan2(delta[1], delta[0]) - yaw + np.pi) % (2 * np.pi) - np.pi
    return abs(float(bearing)) <= half_angle


def holonomic_cmd(xy, yaw, wp, face_xy, speed, stop_dist, brake_dist, turn_kp):
    """[vx, vy, yaw_rate] that translates in the base frame straight towards
    `wp` while turning to face `face_xy`. For a short hop the reference law's
    turn / drive / turn costs more ground in WBC drift (0.2-0.5m per in-place
    turn) than the hop itself. Slow, with the simultaneous turn capped.
    """
    delta = np.asarray(wp, dtype=np.float64) - np.asarray(xy, dtype=np.float64)
    ...
    if dist <= stop_dist:
        ...
        spd = speed * (3 * t * t - 2 * t * t * t)
    spd = 0.0 if dist <= stop_dist else min(max(spd, HOLONOMIC_MIN_SPEED), HOLONOMIC_MAX_SPEED)
    c, s_ = np.cos(yaw), np.sin(yaw)
    lx, ly = c * delta[0] + s_ * delta[1], -s_ * delta[0] + c * delta[1]
    # Safety net for drift during the hop: never more lateral than the cone allows.
    lx = max(lx, 1e-6)
    ly = float(np.clip(ly, -lx * np.tan(HOLONOMIC_MAX_BEARING), lx * np.tan(HOLONOMIC_MAX_BEARING)))
    ln = max(np.sqrt(lx**2 + ly**2), 1e-6)
    desired = np.arctan2(face_xy[1] - xy[1], face_xy[0] - xy[0])
    ye = (desired - yaw + np.pi) % (2 * np.pi) - np.pi
    ang = float(np.clip(turn_kp * ye, -HOLONOMIC_MAX_TURN, HOLONOMIC_MAX_TURN))
...
def sample_standoff_pose(
    ...
    if not occ.any_free_in_annulus(tgt, r_min, r_max):
        return None, None
    here = np.asarray(here_xy, dtype=np.float64)[:2]
    # Lowest cost wins: walk distance, plus RADIUS_WEIGHT x distance from the
    # mid-annulus radius (both edges are marginal for the arm), plus, with
    # `here_yaw`, HEADING_WEIGHT per radian of bearing (a goal ahead is a
    # forward hop; anything else is an in-place turn). `min_dist_from_here`
    # keeps a retry from "walking" 6cm and grasping from the same spot.
    ...
    for _ in range(attempts):
        ...
        d = xy - here
        if np.linalg.norm(d) < min_dist_from_here:
            continue
        cost = float(np.linalg.norm(d)) + RADIUS_WEIGHT * abs(
            float(np.linalg.norm(xy - tgt)) - r_mid
        )
        if here_yaw is not None and np.linalg.norm(d) > 1e-6:
...
class GraspPlanner:
    ...
    def _solve_ik(self, pos, rot=None, hand=None):
        ...
        return self._task.env.robot.solve_scene_ik(
            pos,
            rot=rot,
            hand=hand,
            ik_joints=ik_joints,
    ...
    def _path_is_clear(self, dc, grasp_T, hand):
        ...
        env = task.env if task is not None else None
    ...
    def plan(self, info):
        ...
        self._target_body_id = (
            task.target.body_id if task is not None and task.target is not None else -1
        )
        ...
        if vg is not None and len(vg) > 0:
            ...
            if ik_results:
                ...
                trace_log.info("[G1_MOLMO_TRACE] plan(): 0 candidates survived reach pre-filter")
            if grasp_T is None:
                trace_log.info(
    ...
    def refresh_grasp_for_current_object(self):
        task = getattr(self, "_task", None)
        if task is None or getattr(self, "_grasp_local", None) is None:
            return False
        tgt = task.target
        if tgt is None:
            return False
        env = task.env
        if hasattr(task, "grasp_frame_pose"):
...
class G1PickAgent:
    ...
    def set_task(self, task):
        """The task is the policy's counterpart, as in molmo_spaces' own
        `BaseMujocoTask.register_policy`. It outlives scene reloads (the env's
        MjModel/MjData do not), so this only has to be called once -- but
        `setup()` rebuilds `_grasp_planner`, so that one re-reads `_task` from
        here at the end of every setup(). Order-independent in both
        directions as a result.
        """
        self._task = task
        planner = getattr(self, "_grasp_planner", None)
    ...
    def execute_action(self, action):
        ...
        """
        return self._task.env.robot.execute_action(action)
    ...
    def _set_groot_defaults(self):
        ...
        """
        return self._task.env.robot._set_groot_defaults()
    ...
    def setup(self, model, data, prefix="robot_0/"):
        ...
        self._grasp_planner._task = self._task
    ...
    def precheck_grasp(self, info):
        env = self._task.env if self._task is not None else None
        if env is None:
            return True
    ...
    def _plan_path(self, info):
        ...
        if path:
            ...
            self._holo_final = False
            if self.direct_walk:
                self._waypoints = prune_waypoints(self._waypoints, self._xy())
                self._holo_final = (
                    len(self._waypoints) == 1
                    and float(np.linalg.norm(self._waypoints[0] - self._xy()))
                    < HOLONOMIC_MAX_START_DIST
                    and goal_in_forward_cone(self._xy(), self._yaw(), self._waypoints[0])
                )
            self._has_path = True
            self._walk_trace_mode = None
            self._walk_trace_wp = -1
            self._walk_path_len = 0.0
            self._walk_last_xy = self._xy().copy()
            trace_log.info(
                "[G1 walk] planned from %s (yaw %.2f) to goal %s via %d waypoints: %s",
                np.round(self._xy(), 2),
                self._yaw(),
                np.round(self._target_xy, 2),
                len(self._waypoints),
                [tuple(np.round(w, 2)) for w in self._waypoints],
            )

    def _trace_walk(self, xy, yaw):
        """Accumulate the walked path length (diagnostics only)."""
        last = getattr(self, "_walk_last_xy", None)
        if last is not None:
            self._walk_path_len = getattr(self, "_walk_path_len", 0.0) + float(
                np.linalg.norm(xy - last)
            )
        self._walk_last_xy = np.asarray(xy, dtype=np.float64).copy()

    def _trace_walk_mode(self, mode, xy, yaw, heading_err):
        """Log the walk's control mode whenever it changes (diagnostics only)."""
        if mode == getattr(self, "_walk_trace_mode", None):
            return
        self._walk_trace_mode = mode
        t = self._task.env.time if getattr(self, "_task", None) is not None else float("nan")
        trace_log.info(
            "[G1 walk] t=%.2fs %s at %s yaw %.2f (heading err %.2f), walked %.2fm so far",
            t,
            mode,
            np.round(xy, 2),
            yaw,
            heading_err,
            getattr(self, "_walk_path_len", 0.0),
        )
    ...
    def _update_nav_command(self):
        ...
        if self._facing:
            ...
            if face is not None:
                ...
                if abs(ye) > self.FACE_TOL:
                    self._trace_walk_mode("facing object", xy, yaw, ye)
                    ...
                    return
            self._arrived = True
            self._trace_walk_mode("arrived", xy, yaw, 0.0)
            self._low_level._cmd[:] = 0
            return
        ...
        if final and dist <= stop_dist:
            self._facing = True
            self._trace_walk_mode("at final waypoint", xy, yaw, 0.0)
            self._low_level._cmd[:] = 0
            return
        if final and getattr(self, "_holo_final", False):
            face = self._object_xy if self._object_xy is not None else wp
            self._trace_walk_mode("holonomic hop to goal", xy, yaw, 0.0)
            self._low_level._cmd[:] = holonomic_cmd(
                xy, yaw, wp, face, self._speed, stop_dist, BRAKE_DIST, self.TURN_KP
            )
            return
        ye = (np.arctan2(delta[1], delta[0]) - yaw + np.pi) % (2 * np.pi) - np.pi
        if abs(ye) > self.FACE_WP_TOL:
            self._trace_walk_mode(f"turning to waypoint {self._wp_idx}", xy, yaw, ye)
            self._low_level._cmd[:] = [
                0,
                0,
            ...
            return
        self._trace_walk_mode(f"driving to waypoint {self._wp_idx}", xy, yaw, ye)
        if final:
            if dist <= stop_dist:
                spd = 0.0
            elif dist >= BRAKE_DIST:
    ...
    def perturb_action_for_rollout(self, action):
        ...
        if not self._waypoints or np.linalg.norm(out[0:3]) < 1e-6:
            return out
        rng = self._task.env.np_random if getattr(self, "_task", None) is not None else np.random
    ...
    def _solve_ik_wbc(self, target_pos, target_rot=None, avoid_self_collision=False):
        ...
        return self._task.env.robot.kinematics_wbc(
            target_pos,
    ...
    def _install_closer_waypoint(self, info):
        ...
        r_min = float(getattr(task, "_grasp_spawn_radius_min", 0.25))
    ...
    def _refresh_info_target_pose(self, info):
        ...
        if self._task is None or info is None:
            return
        tgt = self._task.target
        if tgt is None:
            return
        ...
        try:
            if task is not None and hasattr(task, "grasp_frame_pose"):
                pos, quat = task.grasp_frame_pose(self._task.env.scene)
            else:
                pos = tgt.position(self._task.env.scene.data)
                quat = tgt.quat(self._task.env.scene.data)
        except Exception:
            return
    ...
    def _log_trace(self, new_phase):
        ...
        t = (
            getattr(self._task.env, "time", None)
    ...
    def reset(self, info):
        ...
        self._info = info
        # The walk timeout counts from here, not from sim time zero: in a
        # datagen episode the two coincide, but InteractiveShell picks start
        # after minutes of earlier skills, and against absolute time the walk
        # phase was declared timed out on its very first tick.
        self._walk_t0 = self._task.env.time if getattr(self, "_task", None) is not None else 0.0
        ...
        if self._start_at_pregrasp and self._pregrasp_joints is not None and self._task is not None:
            self._task.env.robot.apply_arm_pose(self._pregrasp_joints)
            ...
            if init_upper is not None and self._task is not None:
                self._task.env.robot.apply_upper_pose(init_upper)
                self._low_level._target_q[15:] = self._low_level._data.qpos[
                    self._low_level._jqpos[15:]
                ].astype(np.float32)
                mujoco.mj_forward(self._low_level._model, self._low_level._data)
    ...
    def sample_actions(self, obs):
        ...
        if (
            ...
            if cur_dist < min_dist - 0.02:  # 2cm improvement counts as progress
                ...
                self._stall_last_progress_t = self._task.env.time
            elif (
                self._task.env.time - getattr(self, "_stall_last_progress_t", self._task.env.time)
                > 0.5
            ):
                self._arrived = True
                self._facing = True
        ...
        if (
            ...
            and self._grasp_phase == PHASE_IDLE
            and self._task is not None
            and self._task.env.time - getattr(self, "_walk_t0", 0.0) > self._walk_timeout_s
    ...
    def get_action_chunk(self, observation):
        """Always a single-action chunk; render decimation lives in
        G1PickPlannerPolicy.get_action_chunk, where there is a render to defer."""
        return [self.sample_actions(observation)]

    # molmo_spaces' PlannerPolicy interface, as read-only views onto state
    # sample_actions() already maintains.
    @property
    def planners(self):
...
class _NativeEnvView:
    """Presents molmo_spaces' `CPUMujocoEnv` through the attributes the
    reference policy reads off `task.env`.
    ...
    def __init__(self, task, np_random):
        self._task = task
...
class _NativeTaskView:
    """A molmo_spaces `BaseMujocoTask` seen through the members the reference
    policy reads off `G1Task`: `target`, `env`, the grasp-retry radius. No
    `grasp_frame_pose`: its absence is what selects the pick (not open) path.
    """

    def __init__(self, task, target_obj, np_random, grasp_spawn_radius_min: float = 0.25):
        self.target = _NativeTargetView(target_obj)
        self.env = _NativeEnvView(task, np_random)
...
class G1PickPlannerPolicy(PickPlannerPolicy):
    ...
    def _nav_maps(self):
        """(goal-sampling map, A*-planning map), g1_molmo's occ/occ_safe, at the
        sampler's `robot_safety_radius` so the env's cached map is reused.
        Cached here: the dilation and the A*'s coarsened copy are expensive.
        """
        env = self.task.env
    ...
    def _sample_goal_pose(self, obj, occ, nav_occ):
        """Sample a standoff pose to grasp the object from -- see
        `sample_standoff_pose`, shared with FetchManBasePlannerPolicyPort so
        InteractiveShell's nav_to parks the robot where this pick would have
        walked to anyway. Returns (xy, yaw), or (None, None) if no candidate
        survives -- callers fall back to standing put, which is what this
        policy did unconditionally before nav existed.
        """
        r_min, r_max = self.policy_config.goal_standoff_radius_range
        # First attempt: the cheap pose, ahead of the robot. A retry (see
        # force_standoff_walk) is asked for because a grasp from the cheap pose
        # failed, so it drops the heading preference -- letting the approach
        # side change -- and must move at least 0.15m.
        retry = self.policy_config.force_standoff_walk
        return sample_standoff_pose(
            occ,
            nav_occ,
    ...
    def _build_info(self) -> dict:
        """The reference policy's `info` dict from native state.
        `target_object_pose` is a 7-vector and `valid_grasps` are object-local
        (get_pickup_grasps returns world-frame candidates, mapped back here).
        """
        ...
        # Walk goal, measured from the controller's _xy() (what reset()
        ...
        r_min, r_max = self.policy_config.goal_standoff_radius_range
        standoff = float(np.linalg.norm(tgt_xy - xy))
        forced = self.policy_config.force_standoff_walk
        if forced or not r_min <= standoff <= r_max:
            goal_xy, goal_yaw = self._sample_goal_pose(obj, occ, nav_occ)
            why = (
                "retrying from a fresh pose"
                if forced
                else f"outside the {r_min:.2f}-{r_max:.2f}m standoff annulus"
            )
            if goal_xy is None:
                log.warning(
                    "G1 pick: standing %.2fm from the object, %s, and no reachable "
                    "standoff pose was found -- grasping from here.",
                    standoff,
                    why,
                )
            else:
                log.info(
                    "G1 pick: standing %.2fm from the object, %s -- walking to a %.2fm "
                    "standoff first.",
                    standoff,
                    why,
                    float(np.linalg.norm(tgt_xy - goal_xy)),
                )
        if goal_xy is None:
    ...
    def get_action(self, observation):
        ...
        if self._agent.done and getattr(self, "_last_live_flat", None) is not None:
            ...
            # live upper-body command instead, base stopped.
            flat = self._last_live_flat.copy()
            flat[ACT_CMD] = 0.0
        else:
            self._last_live_flat = flat.copy()
        self._publish_grasp_pose()
        return self._flat_to_move_groups(flat)

    # Render every RENDER_DECIM ticks: step_chunk polls the full sensor suite
    # per tick otherwise. The ticks in between are driven for real through
    # task._apply_action so each sample_actions() sees fresh mj_data
    # (precomputing 20 actions from one observation starved the IK), and the
    # gait clock keeps its 1:1 ratio of get_action to _apply_action.
    RENDER_DECIM = 20

    def get_action_chunk(self, observation):
        """Drive RENDER_DECIM-1 ticks of physics here and return a one-action
        chunk for step_chunk to apply and render. Walk phase included: its
        feedback is read from live mj_data, not from `observation`.
        """
