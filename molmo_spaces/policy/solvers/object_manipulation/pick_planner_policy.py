...
class PickPlannerPolicy(BaseObjectManipulationPlannerPolicy):
    def __init__(self, config, task) -> None:
        ...
        # Tasks that spawn dynamic objects (e.g. mug-ball) set a non-zero settle
        # duration on their task config; everything else defaults to 0 (no settling).
        self._settle_duration = getattr(config.task_config, "scene_settle_duration", 0.0)

    def reset(self, reset_retries: bool = True):
        if self._has_settled or self._settle_duration <= 0:
            ...
            self._settling = True
            self._settle_start_time = None
            log.info(f"[PICK PLANNER] Waiting {self._settle_duration}s for scene to settle")
    ...
    def _compute_target_poses(self) -> dict[str, np.ndarray]:
        ...
        candidate_grasps = get_pickup_grasps(
            ...
            # semantic_grasp_pick restricts to grasps classified semantically good for
            # this object (the prompt_level ablation target).
