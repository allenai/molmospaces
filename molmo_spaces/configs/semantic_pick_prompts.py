...
Categories covered match those present in
/weka/prior/aguru/datasets/semantic_grasp_pick_bench_13APR.
"""
...
def get_semantic_pick_prompt(category: str, object_name: str, level: int) -> str:
    ...
    if level not in VALID_PROMPT_LEVELS:
        raise ValueError(
            f"prompt_level must be one of {VALID_PROMPT_LEVELS}, got {level!r}"
        )
