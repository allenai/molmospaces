...
class NavToObjTask(BaseMujocoTask):
    ...
    def __init__(self, env: BaseMujocoEnv, exp_config: MlSpacesExpConfig) -> None:
        ...
        # Throttle the per-step progress log to roughly once per simulated second.
        self._progress_log_every = max(1, round(1000 / exp_config.policy_dt_ms))
        self._last_progress_log_step: int | None = None
    ...
    def reset(self):
        ...
        self._last_progress_log_step = None
        return super().reset()
    ...
    def judge_success(self) -> bool:
        ...
        if not success and self._should_log_progress():
            distance = self.calculate_distance(0)
            object_visible = self.check_object_visible(0)
            log.info(f"[Walking] Distance: {distance:.2f}m, Object visible: {object_visible}")

        return success

    def _should_log_progress(self) -> bool:
        """True at most once per `_progress_log_every` policy steps (~1s of sim time)."""
        step = self.episode_step_count
        if step % self._progress_log_every != 0 or step == self._last_progress_log_step:
            return False
