    ...
    def _sample_trajectory_obstacles(
        ...
        for _ in range(task_sampler_config.num_trajectory_obstacles):
            ...
            for _ in range(max_point_attempts_per_obstacle):
                ...
                if self._cached_thormap is not None:
                    # is_free, not check_collision: the latter's name is inverted
                    # (it returns True = free, see its own docstring), so testing
                    # it directly rejected exactly the navigable cells we want.
                    # Out-of-map candidates read as not-free, which is the safe
                    # direction here.
                    if not self._cached_thormap.is_free(candidate_point_xy):
