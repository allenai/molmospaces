...
class PackingTask(PickAndPlaceTask):
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        # Names of packing objects currently supported by the receptacle,
        # refreshed on every get_info() call.
        self.objects_in_receptacle: set[str] = set()
        ...
        for i in range(self._env.n_batch):
            ...
            for obj_name in packing_names:
                ...
                if supported:
                    ...
                    # Fallback: contact check + raycast XY. The AABB footprint fallback
                    # is disabled: an open box's AABB includes its flaps, so it would
                    # count objects sitting beside the box as packed.
