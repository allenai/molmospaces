...
class FisheyeRenderer:
    ...
    def render_mask(self, data, renderer, robot_geom_ids):
        """Render the binary silhouette of the geoms in `robot_geom_ids` through the
        same fisheye projection used by `render`. Returns a (H, W) uint8 mask
        (255 = robot pixel, 0 = background) at the renderer's output resolution.
