...
class PickTask:
    ...
    def is_terminated(self, scene) -> bool:
        ...
        (see tasks/open.py's OpenTask.is_terminated for the same shape on the
        sibling task). Gold's PickTask originally defined neither this nor
        is_success, relying entirely on G1Env's hasattr-gated reward-
        threshold fallback (see env_g1ms.py's is_terminal/judge_success) --
        this makes that same threshold an explicit method on the task
        itself instead, same 0.1 lift-height fallback value.
        """
