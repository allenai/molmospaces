...
class ObjectPointInCameraSensor(Sensor):
    """Analytic pinhole projection of a task object's 3D position into each
    configured camera -- normalized (u, v), or (-1, -1) if the object is
    behind the camera. Uses the camera's known intrinsics/extrinsics rather
    than a render, so it does not handle occlusion (or visibility in
    general): the point is reported even when the object is hidden or off
    frame. Use ObjectImagePointsSensor instead when the object's actual
    on-screen extent matters.
    """
