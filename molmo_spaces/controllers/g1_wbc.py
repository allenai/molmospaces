...
# Named slices of the flat 15-element action execute_action() parses.
# Width of the flat legs_waist target (see LegsWaistController.set_target).
...
class WbcController:
    ...
    # controllers/abstract.py's Controller surface, for molmo_spaces' own
    # Robot.reset/set_stationary; the reference stack set_targets every tick
...
class LegsWaistController(WbcController):
    ...
    def set_target(self, target):
        # Two calling conventions, told apart by length: the reference stack's
        # (cmd3, height, waist3) and molmo_spaces' flat [vx, vy, yaw_rate,
        # height, waist(3)].
...
def flat15_to_move_groups(flat, gripper_mg_id: str = "right_gripper") -> dict:
    """gold's flat-15 action (ACT_* slices) -> molmo_spaces' move-group dict."""
    flat = np.asarray(flat)
    return {
        "legs_waist": np.concatenate([flat[ACT_CMD], [flat[ACT_HEIGHT]], flat[ACT_WAIST]]),
        "right_arm": flat[ACT_ARM],
        gripper_mg_id: np.array([flat[ACT_GRIP]]),
    }
...
class G1Controller:
    ...
    def execute_action(self, action):
        """Parse the flat 15-dim action, set_target on each controller, then
        compute and write ctrl in gold's order: legs_waist torque from the
        previous call's _target_lower first, then the WBC inference that
        updates it. That ordering is load-bearing.
        """
