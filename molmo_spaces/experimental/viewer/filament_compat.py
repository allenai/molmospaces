...
class MjFilamentRenderer(MjAbstractRenderer):
    def __init__(
        ...
        self._window = window.Window("FilamentRenderer", width, height, "opengl_headless")

        self._mjv_scene = mj.MjvScene(model=model, maxgeom=max_geom)
        self._mjv_scene_option = mj.MjvOption()
        self._mjv_scene_option.sitegroup *= 0
        self._mjv_scene.flags[mj.mjtRndFlag.mjRND_SHADOW] = True

        self._ctx = mjrf.Context(
            mjrf.ContextConfig(graphics_api=mjrf.GraphicsApi.GRAPHICS_API_OPENGL)
        )
        self._rf_renderer = renderer.Renderer(self._ctx)  # pyright: ignore[reportAttributeAccessIssue]

        self._rf_model_objs = mjrf.ModelObjects(self._ctx, model)
        self._rf_scene = self._rf_renderer.scene("scn")
        self._rf_scene.configure_from_model(model)
        self._rf_model_lights = mjrf.ModelLights(self._rf_scene, self._rf_model_objs)
        self._rf_model_renderables = mjrf.ModelRenderables(self._rf_scene, self._rf_model_objs)
        self._rf_renderer.view(
            "view", scene="scn", target="out", draw_mode=mjrf.DrawMode.DRAW_MODE_DEFAULT
        )
        self._rf_renderer.view(
            "view-depth", scene="scn", target="out-depth", draw_mode=mjrf.DrawMode.DRAW_MODE_DEPTH
        )
        self._rf_renderer.view(
            "view-segid",
            scene="scn",
    ...
    def render(
        ...
        *,
        ...
        original_flags = self._mjv_scene.flags.copy()
        ...
        out_shape = (height, width) if self._depth_rendering else (height, width, 3)
        out_dtype = np.float32 if self._depth_rendering else np.uint8
        ...
        if self._depth_rendering:
            self._rf_renderer.target(
                "out-depth", (self._window.GetWidth(), self._window.GetHeight())
            )
            self._rf_renderer.render()

            out = self._rf_renderer.get_image("out-depth").__array__()
            ...
            # Reset scene flags.
            np.copyto(self._mjv_scene.flags, original_flags)

        elif self._segmentation_rendering:
            self._rf_renderer.target(
                "out-segid", (self._window.GetWidth(), self._window.GetHeight())
            )
            self._rf_renderer.render()

            out = self._rf_renderer.get_image("out-segid").__array__()
            ...
            visible_geoms = [
                g for g in self._mjv_scene.geoms[: self._mjv_scene.ngeom] if g.segid != -1
            ]
            ...
            self._rf_renderer.target("out", (self._window.GetWidth(), self._window.GetHeight()))
            self._rf_renderer.render()

            out = self._rf_renderer.get_image("out").__array__()
    ...
    def upload_textures(self) -> None:
        ...
        # TODO(wilbert): must implement this part using the newer render API
    ...
    def update(
        ...
        scene_option = scene_option or self._mjv_scene_option
        mj.mjv_updateScene(
            self.model,
            data,
            scene_option,
        ...
        self._rf_model_lights.update(data)
        self._rf_model_renderables.update(data)
        self._rf_renderer.update_camera("view", mj.mjv_camera2GLCamera(self._model, data, camera))
        self._rf_renderer.update_camera(
            "view-depth", mj.mjv_camera2GLCamera(self._model, data, camera)
        )
        self._rf_renderer.update_camera(
            "view-segid", mj.mjv_camera2GLCamera(self._model, data, camera)
        )
