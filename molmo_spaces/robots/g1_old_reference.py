"""Superseded independent rewrite of the G1 humanoid (was robots/g1.py).

`molmo_spaces/robots/g1.py` is now the reference-derived implementation (a
reshaped fork of g1_molmo's own components/robot.py, verified byte-identical
to the gold pick rollout). This module is the earlier, independently-written
version, kept because `G1Config` and the native pick/nav pipeline still
construct it.

The two are NOT drop-in interchangeable:
  - constructor: `(mj_data, exp_config)` here vs `(model, data, ...)` there
  - this one is config-driven and owns MoveGroup/Controller objects; that one
    owns its low-level WBC controller and needs an `env` back-reference
  - `kinematics` is a property here; there it is too, but the full-scene solve
    is a separate `solve_scene_ik()` method

Known behavioural gaps versus the reference implementation, found by diffing
the two (these are why its grasp does not contact the object):
  - arm joints keep the MJCF's actuatorfrcrange (+-88 N*m) and zero
    dof_damping; the reference raises both (+-400, damping 3.0)
  - action noise is enabled by default and perturbs the IK-solved arm command
  - the height-smoothing state in FetchmanPickPlannerPolicy is not carried
    forward, so the pelvis never crouches toward the IK's solved height
"""
