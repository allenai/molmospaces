...
class G1HoloBaseGroup(FreeJointRobotBaseGroup):
    """Mocap-weld-driven pelvis base for G1Config(use_holo_base=True), as
    FloatingRUMBaseGroup: a weld pulls the free-joint pelvis to a commanded
    [x, y, z, quat] pose, decoupling base motion from the legs."""
...
class G1LegsWaistGroup(MJCFFrameMixin, SimplyActuatedMoveGroup):
    """The 12 leg joints + 3-DOF waist as one 15-DOF group: the whole-body
    controller computes one coupled law across both. Joint order must match
    the walking policy's training-time order (legs first, then waist)."""
...
class G1RobotView(RobotView):
    ...
    def get_ik_excluded_movegroup_ids(self) -> list[str]:
        """Exclude legs/waist, but not the pelvis, from generic arm-reach IK.
        With both unlocked as redundant DOF the solver wandered the leg joints
        and failed every grasp; a small pelvis shift is a sensible way to
        extend reach, bipedal leg motion is not."""
        return ["legs_waist"]

    def is_close_to(
        self, move_group_ids: list[str], target_pose: list, threshold: float = 0.1
    ) -> bool:
        """Is the planar base pose within `threshold` of the target. The default
        governs "waypoint reached" for G1 nav; 0.1 because the WBC's tracking
        plateaus around 0.07, so 0.05 was never reached."""
