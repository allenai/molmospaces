...
class TestFrankaPickDroidDataGenConfig:
    """Tests for FrankaPickDataGenConfig."""
...
class TestFrankaPickRandomizedDataGenConfig:
    """Tests for FrankaPickDataGenConfig."""

    @pytest.fixture
    def config(self):
        return FrankaPickRandomizedDataGenConfig()

    def test_instantiation(self, config):
        """Test that the config can be instantiated."""
        assert config is not None
        assert isinstance(config, FrankaPickRandomizedDataGenConfig)
        assert isinstance(config, MlSpacesExpConfig)
    ...
    def test_output_dir_and_tag(self, config):
        ...
        assert (
            "franka" in config.tag.lower()
            and "pick" in config.tag.lower()
            and "randomized" in config.tag.lower()
        )
...
class TestConfigUniqueness:
    ...
    def _get_franka_configs(self):
        """Get all registered Franka configs dynamically."""
        all_config_names = list_available_configs()
        franka_config_names = [name for name in all_config_names if "franka" in name.lower()]

        # Instantiate all Franka configs
        configs = []
        for name in franka_config_names:
            config_cls = get_config_class(name)
            configs.append(config_cls())

        return configs

    def _get_rum_configs(self):
        """Get all registered RUM configs dynamically."""
        all_config_names = list_available_configs()
        rum_config_names = [name for name in all_config_names if "rum" in name.lower()]

        # Instantiate all RUM configs
        configs = []
        for name in rum_config_names:
            config_cls = get_config_class(name)
            configs.append(config_cls())
    ...
    def test_all_configs_have_unique_output_dirs(self):
        """Test that all Franka configs have unique output directories."""
        configs = self._get_franka_configs() + self._get_rum_configs()

        # Ensure we found at least some configs
        assert len(configs) > 0, "Should find at least one Franka config in registry"
    ...
    def test_all_configs_have_unique_tags(self):
        """Test that all Franka configs have unique tags."""
        configs = self._get_franka_configs() + self._get_rum_configs()
        ...
        MAX_TAG_LENGTH = 8000
        # Test all tags are less than 12 characters long
        long_tags = [t for t in tags if len(t) > MAX_TAG_LENGTH]
        assert not long_tags, f"Tags too long: {long_tags}"
