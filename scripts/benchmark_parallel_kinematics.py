...
def main():
    ...
    kinematics_solvers: dict[str, ParallelKinematics] = {
        ...
        "jax (cpu)": FrankaParallelKinematics(robot_config),
        "warp (cpu)": SimpleWarpKinematics(robot_config),
        "warp (cuda)": SimpleWarpKinematics(robot_config, device="cuda"),
    }
    ...
    poses = np.repeat(cpu_kinematics.fk(robot_config.init_qpos, np.eye(4))["gripper"][None], max_batch_size, axis=0)
    np.random.seed(42)
