...
class Scene:
    ...
    def __init__(
        ...
        spec_ops.add_base_weld(spec, robot_prefix=self._robot_prefix)

        self._spec = spec
        self.model = spec.compile()
        self.data = mujoco.MjData(self.model)
        # Anti-tumble damping, rate-capped per DOF (flat 1.0 NaN-explodes gram-scale objects).
        spec_ops.cap_freejoint_damping(self.model)
        ...
        if self._scene_texture_paths:
            from molmo_spaces.env.randomization.texture import classify_scene_geom
    ...
    def objects(self):
        ...
        for body_id in om.top_level_bodies():
            ...
            if (
                ...
                or name == spec_ops.grasp_probe_body_name(0)
            ):
                continue
            names.append(name)
    ...
    def _optimize(self, spec, metadata, mobile_regex, articulated_regex=None):
        ...
        static geometry -- see spec_ops.freeze_non_mobile_bodies, which the
        native scene build applies too (task_sampler._apply_spec_ops)."""
        spec_ops.freeze_non_mobile_bodies(
            spec,
            metadata,
