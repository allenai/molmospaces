...
def get_args():
    parser = argparse.ArgumentParser(description="fetchman single-worker collector")
    parser.add_argument(
...
def main():
    ...
    cfg.seed = args.seed
    cfg.use_passive_viewer = args.render and not args.cv2

    task_sampler = make_task_sampler(cfg)
    raw_env = task_sampler.env
    task = task_sampler.task
    ...
    if args.record:
        ...
        recorder = LeRobotRecorder(
            ...
            env=raw_env,
            # The recorder stores gold's flat-15, which is what the
            # controller emits; the task's own action is a move-group dict.
        ...
        while episode < args.episodes:
            ...
            while not stop[0]:
                try:
                    ...
                    print(f"[main] sample_task failed: {e}; retrying", flush=True)
                    continue
            ...
            while raw_env.time < time_limit and not stop[0]:
                ...
                obs, r, terminated, truncated, info = task.step(flat15_to_move_groups(exec_action))
                agent.set_step_info(info)
                if args.render:
                    task.sync_viewer()
                    _cv2_render()
                if args.render and task.consume_skip_episode():
                    break
                if terminated or truncated or agent.done:
                    break
                if args.render and not args.cv2 and not raw_env.viewer_running:
                    stop[0] = True
                    break
                if args.realtime:
