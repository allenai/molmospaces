...
Run once per stack, then diff:
    ...
    cd ~/code/g1_molmo   # the reference checkout
    conda run -n g1_molmo python \
        <this repo>/fetchman/scripts/check_texture_parity.py \
...
def run(stack, out_prefix, seed):
    if stack == "gold":
        ...
        from molmospaces.env_g1ms import make_env as make_task_sampler
    else:
        from fetchman.configs.bowl_fetchman import get_config
        from fetchman.tasks.pick_task_sampler_g1ms import make_task_sampler

    if stack == "gold":
        # gold is still on ml_collections; its config is a flat ConfigDict.
    ...
    sampler = make_task_sampler(cfg)
    raw = getattr(sampler, "env", sampler)
    sampler.sample_task()

    # Pool discovery is the sampler's; the applied assignment lands on the env.
    pools = getattr(sampler, "_texture_pools", None) or raw._texture_pools
