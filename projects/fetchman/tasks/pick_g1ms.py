...
from fetchman.tasks.task_g1ms import G1Task
...
class PickTask(G1Task):
    """Lift the target object off its support.

    Everything about running the rollout (reset/step/obs/termination) is
    G1Task's; this class supplies only the pick-specific hooks -- which objects
    are candidates, which one is the target, the lift reward, and the info the
    policy is prompted with.
    """

    def __init__(self, env, exp_config):
        super().__init__(env, exp_config)
        tc = exp_config.task_config
        # The candidate-object filter is the sampler's (it also selects which
        # scene those objects come from), so it is read from that section.
        self._object_regex = exp_config.task_sampler_config.objects or ".*"
        self._randomize_object = tc.randomize_object
        self._object_noise = tc.object_noise
        self._prompt_sampler = PromptSamplerSimple(config=get_prompt_config())
        self._prompt = ""
    ...
    def is_terminated(self, scene) -> bool:
        """Matches molmo_spaces' BaseMujocoTask.is_terminal()-style contract
        (see tasks/open_g1ms.py's OpenTask.is_terminated for the same shape on
        the sibling task). Gold's PickTask defined neither this nor is_success,
        relying on a reward-threshold fallback in the env's own step(); the
        threshold is the same 0.1 lift height, now an explicit method here.
        """
        return self.compute_reward(scene) > 0.1

    def is_success(self, scene) -> bool:
        """See is_terminated's docstring -- same relationship to the
        original 0.04 lift-height success fallback."""
        return self.compute_reward(scene) > 0.04
