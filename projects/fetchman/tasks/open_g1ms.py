...
class OpenTask(G1Task):
    """See the module docstring. Supplies the open/close-specific hooks over
    G1Task's rollout machinery, same division as PickTask."""
    ...
    def __init__(self, env, exp_config):
        ...
        self._task_type = str(tc.task_type).lower()  # "open" | "close"
        self._success_threshold = tc.open_success_threshold
        self._terminate_threshold = tc.open_terminate_threshold
        self._init_open_percent = tc.open_init_percent
        self._require_joint_grasp = tc.open_require_joint_grasp
